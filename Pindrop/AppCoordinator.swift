...
final class AppCoordinator {
    ...
    private func stopRecordingAndTranscribeForQuickCapture() async throws -> AIEnhancementService.EnhancedNote? {
        ...
        let noteEnhancementProvider = settingsStore.currentAIProvider
        let noteEnhancementAPIKey = settingsStore.loadAPIKey(for: noteEnhancementProvider)
        if settingsStore.aiEnhancementEnabled,
           let apiEndpoint = settingsStore.apiEndpoint,
           (!noteEnhancementProvider.requiresAPIKey ||
            (noteEnhancementAPIKey?.isEmpty == false)) {
            do {
    ...
    private func stopRecordingAndTranscribe() async throws {
        ...
        let shouldUsePlaceholderMentions = settingsStore.aiEnhancementEnabled &&
            ...
            (!settingsStore.currentAIProvider.requiresAPIKey ||
             settingsStore.loadAPIKey(for: settingsStore.currentAIProvider)?.isEmpty == false)
        ...
        let loadedAPIKey = settingsStore.loadAPIKey(for: provider)
        if settingsStore.aiEnhancementEnabled,
           let apiEndpoint = settingsStore.apiEndpoint,
           (!provider.requiresAPIKey || (loadedAPIKey?.isEmpty == false)) {
            do {
            ...
            } else if settingsStore.currentAIProvider.requiresAPIKey
               && settingsStore.loadAPIKey(for: settingsStore.currentAIProvider)?.isEmpty != false
            {
